...
) -> Result<railgunners_types::ShieldCiphertext, EventError> {
...
mod tests {
    ...
    fn decodes_v2_transact_event_into_typed_commitment_event() {
        ...
        let railgunners_types::VersionedCommitmentEvent::V2(event) = event else {
    ...
    fn decodes_v3_transact_event() {
        ...
        let railgunners_types::VersionedCommitmentEvent::V3(event) = event else {
    ...
    fn decodes_v3_transaction_event_with_unshield_sentinels() {
        ...
        assert_eq!(
            event.verification_hash().map(railgunners_types::VerificationHash::as_bytes),
            Some(&[0x71; 32])
        );
    }
    ...
    fn decodes_raw_v2_transact_log_into_canonical_event() {
        ...
        let DecodedRailgunLogEvent::Commitment(railgunners_types::VersionedCommitmentEvent::V2(
            event,
        )) = &events[0]
        else {
    ...
    fn decodes_raw_v3_accumulator_log_into_ordered_canonical_events() {
        ...
        let DecodedRailgunLogEvent::Commitment(railgunners_types::VersionedCommitmentEvent::V3(
            transact,
        )) = &events[0]
        else {
        ...
        let DecodedRailgunLogEvent::Nullifier(railgunners_types::VersionedNullifierEvent::V3(
            nullifiers,
        )) = &events[1]
        else {
        ...
        let DecodedRailgunLogEvent::Unshield(railgunners_types::VersionedUnshieldEvent::V3(
            unshield,
        )) = &events[2]
        else {
