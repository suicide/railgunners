...
use railgunners_core::parse_canonical_field_bytes;
use railgunners_types::{Groth16Proof, MerkleNodeHash, MerkleRoot, RailgunTxid};
...
mod tests {
    ...
    use num_bigint::BigUint;
    use railgunners_types::{BN254_SCALAR_FIELD_MODULUS_BYTES, Groth16Proof};
    ...
    fn root(byte: u8) -> railgunners_types::MerkleRoot {
        railgunners_types::MerkleRoot::new([byte; 32])
    }

    fn txid(byte: u8) -> railgunners_types::RailgunTxid {
        railgunners_types::RailgunTxid::new(BigUint::from(byte))
