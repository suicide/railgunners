...
use railgunners_core::decode_railgun_address;
use railgunners_poi::PoiListKey;
use railgunners_types::{Address, RailgunAddress, ViewingPrivateKey};
...
pub fn sign_fee_message(
    ...
    Ok(BroadcasterFeeMessage::new(data.clone(), encoded_data, hex::encode(signature.to_bytes())))
}
...
mod tests {
    ...
    use railgunners_core::{derive_viewing_public_key, encode_railgun_address};
    use railgunners_poi::PoiListKey;
    use railgunners_types::{Address, ChainScope, MasterPublicKey, ViewingPrivateKey};
    ...
    fn verifies_fee_message_signature_with_prefixed_wire_signature() {
        ...
        let signature =
            value["signature"].as_str().unwrap_or_else(|| panic!("signature should be a string"));
