...
use crate::{
    Bip39Mnemonic,
    hd::{
        KeyDerivationError, WalletNode, derive_spending_node, derive_viewing_node, spending_path,
        viewing_path,
    },
};
...
pub struct DerivedWalletKeys {
    index: u32,
    spending_path: crate::hd::DerivationPath,
    viewing_path: crate::hd::DerivationPath,
...
impl DerivedWalletKeys {
    /// Creates a derived wallet-key bundle from explicit components.
    #[must_use]
    pub fn new(
        index: u32,
        spending_path: crate::hd::DerivationPath,
    ...
    /// Returns the wallet index used during derivation.
    #[must_use]
    pub const fn index(&self) -> u32 {
        self.index
    }

    /// Returns the canonical spending derivation path.
    #[must_use]
    pub const fn spending_path(&self) -> &crate::hd::DerivationPath {
        &self.spending_path
    }

    /// Returns the canonical viewing derivation path.
    #[must_use]
    pub const fn viewing_path(&self) -> &crate::hd::DerivationPath {
        &self.viewing_path
    }
...
impl ViewingKeyInspection {
    ...
    #[must_use]
    pub const fn new(viewing_public_key: ViewingPublicKey, nullifying_key: NullifyingKey) -> Self {
        Self { viewing_public_key, nullifying_key }
    }

    /// Returns the derived viewing public key.
    #[must_use]
    pub const fn viewing_public_key(&self) -> &ViewingPublicKey {
        &self.viewing_public_key
    }

    /// Returns the derived nullifying key.
    #[must_use]
    pub const fn nullifying_key(&self) -> &NullifyingKey {
        &self.nullifying_key
    }
...
/// Derives the canonical Railgun wallet keys for a mnemonic and wallet index.
///
/// # Errors
///
/// Returns an error if the wallet index is invalid or if hardened derivation or
/// key derivation fails unexpectedly.
pub fn derive_wallet_keys(
    ...
    let spending_node = derive_spending_node(&seed, wallet_index)?;
    let viewing_node = derive_viewing_node(&seed, wallet_index)?;
...
/// Inspects a viewing private key by deriving its public and nullifying keys.
///
/// # Errors
///
/// Returns an error if nullifying key derivation fails unexpectedly.
pub fn inspect_viewing_private_key(
...
/// Inspects a spending private key by deriving its public key.
///
/// # Errors
///
/// Returns an error if spending public-key derivation fails unexpectedly.
pub fn inspect_spending_private_key(
    private_key: &SpendingPrivateKey,
) -> Result<SpendingPublicKey, KeyDerivationError> {
    derive_spending_public_key(private_key)
}

/// Inspects master-public-key inputs by composing the canonical hash.
///
/// # Errors
///
/// Returns an error if an input integer is not a valid BN254 field element or
/// if Poseidon hashing fails unexpectedly.
pub fn inspect_master_public_key(
    spending_public_key: &SpendingPublicKey,
    nullifying_key: &NullifyingKey,
...
mod tests {
    use super::{
        ...
        derive_viewing_public_key_from_bytes, derive_wallet_keys, inspect_master_public_key,
        inspect_spending_private_key, inspect_viewing_private_key, spending_private_key_from_node,
        viewing_private_key_from_node,
    };
    use crate::{
        Bip39Mnemonic,
        hd::{KeyDerivationError, derive_node_from_str},
    };
    use num_bigint::BigUint;
    ...
    #[test]
    fn derives_wallet_keys_from_mnemonic_and_index() {
        let mnemonic = Bip39Mnemonic::parse(
            "abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon about",
        )
        .unwrap_or_else(|_| panic!("issue mnemonic should parse"));
        let keys = derive_wallet_keys(&mnemonic, 0)
            .unwrap_or_else(|_| panic!("wallet key derivation should succeed"));

        assert_eq!(keys.index(), 0);
        assert_eq!(keys.spending_path().to_string(), "m/44'/1984'/0'/0'/0'");
        assert_eq!(keys.viewing_path().to_string(), "m/420'/1984'/0'/0'/0'");
        assert_eq!(
            hex_encode(keys.spending_private_key().as_bytes()),
            "08b2d974aa7fffd9d068b78c34434c534ddcd9343fcbf5aa12cf78e1a3c1ccb9"
        );
        assert_eq!(
            keys.spending_public_key().x().to_string(),
            "21725194683971601625357993914711234354000760307317172095138789827480990690892"
        );
        assert_eq!(
            keys.spending_public_key().y().to_string(),
            "18185059732936663794890181151638097537207598791675324797050194801074344044960"
        );
        assert_eq!(
            hex_encode(keys.viewing_private_key().as_bytes()),
            "9a9e1ca3b9476dc8500b43f30f34104c92a3eedfd727757ffd0ad15da8e11572"
        );
        assert_eq!(
            hex_encode(keys.viewing_public_key().as_bytes()),
            "df2dfb942aa6fb8cf9fe60d7984cd10b20b59027e677ecb4960d764f7d42408a"
        );
        assert_eq!(
            keys.nullifying_key().value().to_string(),
            "11357301776152573321369788690304620243322420398401862164527624501081803879965"
        );
        assert_eq!(
            keys.master_public_key().value().to_string(),
            "19349903103956176070235423774157995896840157182198600174309409106416294821789"
        );
    }

    #[test]
    fn inspects_viewing_private_key_issue_vector_one() {
        let private_key =
            hex_array::<32>("67d7d19d00e6e3b3517fe68ac46505dd207df6e8fe3aa06ba3face352e7599ef");
        let inspection = inspect_viewing_private_key(&ViewingPrivateKey::new(private_key))
            .unwrap_or_else(|_| panic!("viewing private-key inspection should succeed"));

        assert_eq!(
            hex_encode(inspection.viewing_public_key().as_bytes()),
            "0debf77d8e9436fc07a0dc3fe8bd90c2f592a08cab8dbe5f972a4783465cd6d4"
        );
        assert_eq!(
            inspection.nullifying_key().value().to_string(),
            "12835268173099116305231859677177501123414588269721547120001227054861606950622"
        );
    }

    #[test]
    fn inspects_spending_private_key_by_deriving_public_key() {
        let private_key =
            hex_array::<32>("67d7d19d00e6e3b3517fe68ac46505dd207df6e8fe3aa06ba3face352e7599ef");
        let public_key = inspect_spending_private_key(&SpendingPrivateKey::new(private_key))
            .unwrap_or_else(|_| panic!("spending private-key inspection should succeed"));
    ...
    #[test]
    fn inspects_master_public_key_issue_vector_two() {
        let spending_public_key = SpendingPublicKey::new(
            parse_decimal(
                "15684838006997671713939066069845237677934334329285343229142447933587909549584",
            ),
        ...
        let master_public_key = inspect_master_public_key(&spending_public_key, &nullifying_key)
            .unwrap_or_else(|_| panic!("master public-key inspection should succeed"));
